...
from core.utility import Syserror, check_file, get_file_name, validateStandardPerm
...
class StandardAPIView(APIView):
    ...
    def get(self, request, id=None):
        try:
            perms = request.user.standard_permission
            if id:
                try:
                    filter_criteria = Q(id=id)
                    ...
                    if request.user.role == "User" and not validateStandardPerm(perms, instance.standard_type.lower()):
                        return Response({
                            "success": False,
                ...
                if not standard_type:
                    standard_type = "BIS"
                if request.user.role == "User" and not validateStandardPerm(perms, standard_type.lower()):
                    return Response({
                        "success": False,
                        "message": "Required View  Permission.",
                        "results": None,
    ...
    def put(self, request):
        try:
            ...
            elif instance.standard_type in ["ASTM","AWWA","BRITISH","DIN(GERMAN)","GOST(RUSSIAN)","IEC","ISO","IRST","API","PSN"]:
                division = None
                division_code = None
                committee_code = None
                committee_title = None
    ...
    def get(self, request, id):
        try:
            try:
                ...
                if request.user.role == "User" and not validateStandardPerm(request.user.standard_permission, instance.standard_type.lower()):
                    return Response({
                        "success": False,
                        "message": "Required View  Permission.",
                        "results": None,
...
class SearchRsnGroupApiView(APIView):
    ...
    def get(self, request, vol_id=None):
        try:
            filter_criteria = Q()
            if vol_id:
...
class SearchRsnVolumeApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            exclude_id = request.GET.get("exclude_id")
            if exclude_id:
                instance = RSNVolume.objects.filter(filter_criteria).exclude(id = exclude_id).order_by("volume_no")
            else:
                instance = RSNVolume.objects.filter(filter_criteria).order_by("volume_no")
...
class SearchIPSSTitleApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            exclude_id = request.GET.get("exclude_id")
            if exclude_id:
                instance = IPSSTitle.objects.filter(filter_criteria).exclude(id = exclude_id).order_by("title").distinct()
            else:
    ...
    def get(self, request):
        try:
            ...
            response_data.update(
                {
                    "success": True,
                    "message": "Retrieve archived standard successfully.",
                }
            )
