...
class DrawingCountApiView(APIView):
    permission_classes = [AllowAny]
    authentication_classes = []
    def get(self, request):
        drawing_count = Drawing.objects.filter(is_archive = False).count()
        standard_count = Standard.objects.filter(is_archive = False).count()
        document_count = Manual.objects.filter(is_archive = False).count()
        sir_count = SIR.objects.filter(is_archive = False).count()
        sc_count = StabilityCertification.objects.filter(is_archive = False).count()
        compliance_count = Compliance.objects.filter(is_archive = False).count()
        data = {
            "drawing_count":drawing_count,
            "standard_count":standard_count,
            "document_count":document_count,
            "si_count":sir_count + sc_count + compliance_count,
        }
        response = {
            "success": True,
...
class NoticeApiView(APIView):
    ...
    def get(self, request):
        instance = Notice.objects.first()
        if instance:
            response = {
                "success": True,
                "message": "Notice data retrieved successfully.",
                "results": instance.notice
            }
            return Response(response, status=200)
        else:
            response = {
    ...
    def post(self, request):
        try:
            data = request.data
            notice = data.get("notice", None) or None
            if not notice:
                response = {
                    "success": False,
            ...
            with transaction.atomic():
                ...
                if instance:
                    ...
                    instance = Notice.objects.create(notice = notice)
    ...
    def get(self, request):
        try:
            ...
            response = {
                "success": True,
                "message": "Notice data retrieved successfully.",
                "results": instance.notice if instance else None
            }
            return Response(response, status=200)
...
class SliderOpenApiView(APIView):
    ...
    def get(self, request):
        try:
            instance = Slider.objects.filter(is_published=True).order_by("-created_at")[:50] 
