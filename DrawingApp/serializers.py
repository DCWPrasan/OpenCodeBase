...
class DrawingListSerializer(serializers.ModelSerializer):
    ...
    def get_volume(self, obj):
        ...
        return None
    def get_files(self, obj):
        return obj.files.all().values('id', 'file_name').order_by('file_name') or  []
    
    class Meta:
        model = Drawing
        fields = ["id", "drawing_type", "drawing_number", "revision_version", "work_order_number", "is_layout", "is_archive", "default_description", "department", "unit", "volume", "files"]
        read_only_fields = fields
