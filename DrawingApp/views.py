...
class DrawingApiView(APIView):
    ...
    def get(self, request, id=None):
        try:
            ...
            filter_criteria = Q()
            if request.user.role == "User":
                filter_criteria &= Q(is_approved=True, is_archive=False)
            ...
            if query := request.GET.get("query"): #TODO
            ...
            if request.user.role in ["Admin", "SuperAdmin"] and not any(query_params.values()) and not dwg_status and not file_present and not description and not query:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for file in all_files:
                    ...
                    if old_drawing_files := DrawingFile.objects.filter(drawing=drawing, file_name=file_name):  
                        old_drawingfile = old_drawing_files.first()
                        if file_ext != "DWG":
                            if replacefile:
                                if not dwg_file:
                                    dwg_file = old_drawingfile.dwg_file
                                # old_drawingfile.delete()
                                if old_drawingfile.file:
                                    file_path = old_drawingfile.file.path
                                    if os.path.isfile(file_path):
                                        os.remove(file_path)

                                if old_drawingfile.view_pdf_file:
                                    pdd_path = old_drawingfile.view_pdf_file.path
                                    if os.path.isfile(pdd_path):
                                        os.remove(pdd_path)
                                
                                old_drawingfile.file = file
                                old_drawingfile.file_size=file.size
                                old_drawingfile.dwg_file = dwg_file
                                old_drawingfile.save()
                                convert_TIF_Pdf(old_drawingfile)
                ...
                #file save completed and loop end here
...
class ArchiveDrawingApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            instance = (
                Drawing.objects.select_related('unit', "department", "sub_volume")
...
class BulkArchiveDrawingApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if isDistinct:
                instance = Drawing.objects.select_related('department', 'unit', 'sub_volume').filter(
                filter_criteria).annotate(description_count=Count('description', distinct=True)).order_by('drawing_number_numeric')
            else:
