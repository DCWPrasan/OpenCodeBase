...
class StocksView(APIView, CustomPagination):
    ...
    def get(self, request, id=None):
        try:
            ...
            if barcode := request.GET.get("query"):
                if stock := Stocks.objects.filter(barcode__barcode_no=barcode).first():
                    if stock.quantity > 0:
                        response = {
                            "success": True,
    ...
    def create_stock_in(self, data, user, is_single_stock=False):
        try:
            ...
            barcode = Barcodes.objects.filter(barcode_no=barcode_no).first()
            if not barcode:
    ...
    def get(self, request):
        try:
            if product_barcode_no := request.GET.get("product_barcode_no", None):
                barcode = Barcodes.objects.filter(barcode_no=product_barcode_no).first()
                if not barcode:
                    response = {
                        "success": False,
                        "message": "Invalid barcode",
        ...
        try:
            with transaction.atomic():
                ...
                instance = Employees.objects.create(
                    name=name, email=email, phone=data.get("phone", None)
                )
                response = {
                    "success": True,
