...
class DashboardApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            drawing_log = DrawingLog.objects.select_related('user', 'drawing').filter(user=request.user, action_time__date = datetime.now().date()).order_by('-action_time')

            drawing_log_count = drawing_log.aggregate(
                add=Count("id", filter=Q(status="Add Drawing")),
                update=Count("id", filter=Q(status="Update Drawing")),
                view=Count("id", filter=Q(status="View Drawing")),
                download=Count("id", filter=Q(status="Download Drawing")),
            )
            data = {
                "drawing_log_count": {
                    "add": drawing_log_count["add"] or 0,
                    "update": drawing_log_count["update"] or 0,
                    "view": drawing_log_count["view"] or 0,
                    "download": drawing_log_count["download"] or 0,
                },
            }

            if request.user.is_superuser:
                data['drawing_log'] = DrawingLogListSerializer(drawing_log[:10], many=True).data
                drawing = Drawing.objects.only('id', 'is_approved', 'is_file_present').aggregate(
                ...
                drawing_count = {
                    "pending": drawing["pending"] or 0,
                    "ready_to_approve": drawing["ready_approve"] or 0,
                    "approved": drawing["approved"] or 0,
                    "archived": drawing["archived"] or 0,
                }
                ...
                standard_count = {
                    "pending": standard["pending"] or 0,
                    "approved": standard["approved"] or 0,
                    "archived": standard["archived"] or 0,
                }

                manual = Manual.objects.select_related('user', 'manual').aggregate(
                    pending=Count("id", filter=Q(is_approved=False, is_archive=False)),
                    approved=Count("id", filter=Q(is_approved=True, is_archive=False)),
                    archived=Count("id", filter=Q(is_archive=True)),
                )
                ...
                manual_count = {
                    "pending": manual["pending"] or 0,
                    "approved": manual["approved"] or 0,
                    "archived": manual["archived"] or 0,
                }
                

                sir = SIR.objects.select_related().aggregate(
                    pending=Count("id", filter=Q(is_approved=False, is_archive=False)),
                    approved=Count("id", filter=Q(is_approved=True, is_archive=False)),
                    archived=Count("id", filter=Q(is_archive=True)),
                )
                ...
                sir_count = {
                    "pending": sir["pending"] or 0,
                    "approved": sir["approved"] or 0,
                    "archived": sir["archived"] or 0,
                }

                stability_certificate = StabilityCertification.objects.select_related().aggregate(
                    pending=Count("id", filter=Q(is_approved=False, is_archive=False)),
                    approved=Count("id", filter=Q(is_approved=True, is_archive=False)),
                    archived=Count("id", filter=Q(is_archive=True)),
                )
                
                stability_certificate_count = {
                    "pending": stability_certificate["pending"] or 0,
                    "approved": stability_certificate["approved"] or 0,
                    "archived": stability_certificate["archived"] or 0,
                }

                compliance = Compliance.objects.select_related().aggregate(
                    pending=Count("id", filter=Q(is_approved=False, is_archive=False)),
                    approved=Count("id", filter=Q(is_approved=True, is_archive=False)),
                    archived=Count("id", filter=Q(is_archive=True)),
                )
                ...
                compliance_count = {
                    "pending": compliance["pending"] or 0,
                    "approved": compliance["approved"] or 0,
                    "archived": compliance["archived"] or 0,
                }

                data["drawing_count"] = drawing_count
                data["standard_count"] = standard_count
                data["manual_count"] = manual_count
                data["sir_count"] = sir_count
                data["stability_certificate_count"] = stability_certificate_count
                data["compliance_count"] = compliance_count

            if request.user.role != "User":
                top_10_users = LogInOutLog.objects.filter(message = "Login", user__is_superuser = False).values('user_id', 'user__full_name', 'user__personnel_number').annotate(log_count=Count('user_id')).order_by('-log_count')[:10]
                data["most_user_login"] = top_10_users
...
class VisitorCountApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            online_user = user.filter(Q(last_login__gt = (today-timedelta(hours=8))) | Q(jti_token__isnull = False) ).count()
            data = {
