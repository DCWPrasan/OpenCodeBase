        ...
        try:
            if id:
                try:
                    ...
                    if instance.is_layout and (request.user.role == "User" and not request.user.is_view_layout):
                        return Response({
                            "success": False,
                    ...
                    if instance.drawing_type == "FDR" and (request.user.role == "User" and not request.user.is_design_user):
                        return Response({
                            "success": False,
            ...
            # Retrieving list of user objects according to filter_criteria
            ...
            if query := request.GET.get("query"):
                ...
                if q_drawing_type:
                    filter_criteria &= Q(drawing_type=q_drawing_type)
                    if drawing_number:
                        filter_criteria &= Q(drawing_number__icontains=drawing_number)
                else:
                    filter_criteria &= Q(drawing_number__startswith=query)

            if request.user.role == "User" and not any(query_params.values()) and not query and not dwg_status and not file_present:
                return Response({"success": True, "results": [], "count": 0}, status=200)
            
            if query_params.get('drawing_type') == "FDR" and (request.user.role == "User" and not request.user.is_design_user):
                return Response({"success": True, "results": [], "count": 0}, status=200)
...
class ApproveDrawingApiView(APIView):  # for pending drawing
    ...
    def get(self, request):
        instance = (
            ...
            .filter(is_archive=False, is_approved=False, is_file_present=True)
...
class DownalodDrawingFileApiView(APIView):
    def get(self, request, id):
        try:
            ...
            if (
                instance.drawing.is_layout and request.user.role == "User"
            ) and not request.user.is_view_layout:
                return Response(
                    {"success": False, "message": "Required Permission"}, status=400
                )
            ...
            if action == "download":
                ...
                if dwg_download and not request.user.is_download_drawing:
                    return Response(
                        {
                            "success": False,
        ...
        try:
            ...
            try:
                ...
                if request.user.role == "User":
                    filter_criteria &= Q(is_archive=False, is_approved=True)
                instance = Drawing.objects.get(filter_criteria)
                if instance.is_layout and (request.user.role == "User" and not request.user.is_view_layout) or (instance.drawing_type == "FDR" and not request.user.is_design_user): #TODO
                        response = {
                            "success": False,
                            "message": "Required View Layout Permission.",
                            "results": None,
...
class SearchDrawingApiView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if request.user.role == "User":
                filter_criteria &= Q(is_archive=False, is_approved=True)
            instance = Drawing.objects.filter(filter_criteria).only('drawing_type', 'drawing_number'
            ).annotate(
