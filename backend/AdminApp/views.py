...
from .serializers import (
    ...
    RequisitionItemStockoutSerializer,
    ReservationSerializer,
    ReservationItemStockinSerializer,
    SearchRequisitionsSerializer,
    SearchReservationtionsSerializer,
)
...
class DashboardView(APIView):
    def get(self, request):
        try:
            ...
            data = {
                ...
                "requisition_stats": self.get_requisition_stats(),
    ...
    def get_perishable_stock(self):
        ...
        expiring_stocks = (
            Stocks.objects.filter(
                quantity__gt=0,
                expired_date__range=[today, date_ranges[-1]],
            )
            .values("expired_date")
    ...
    def get_requisition_stats(self):
        today = datetime.now().date()
        return {
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if len(lowStockProduct) > 0:
                    send_low_stock_email_alert(lowStockProduct)
                response = {
                    "success": True,
...
class RequisitionItemForStockoutView(APIView):
    ...
    def put(self, request, requisition_id=None):
        try:
            ...
            with transaction.atomic():
                data = request.data
                requisition_items = data.get("requisition_items", [])
                ...
                for item in requisition_items:
                    ...
                    if not item.get("approve_quantity"):
                    ...
                    employee, created = Employees.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            "phone": requisition.requested_by.mobile_number,
                        },
                    )
                    purpose = requisition_item.purpose
                    out_quantity = 0
                    for i in stocks:
                        quantity = self.get_value_from_stock_list(
                            stock_list, i.id, "quantity"
                        )
                        product = Products.objects.get(id=i.product.id)
                        if requisition_item.product.id != i.product.id:
                            raise ValueError(
            ...
            with transaction.atomic():
                ...
                for item in items:
                    ...
                    if not product_id or not quantity:
                        raise ValueError("Each item must have product and quantity")
                ...
                req.total_quantity = total_quantity
                req.save()
...
class ReservationView(APIView):
    def get(self, request, id=None):
        try:
            ...
            if query := request.GET.get("query"):
                filter_criteria &= Q(
                    Q(reserved_by__name__icontains=query)
                    | Q(reservation_type__icontains=query)
                    | Q(reference_no__icontains=query)
                )
            if status := request.GET.get("status"):
                filter_criteria &= Q(status=status)
...
class ReservationItemForStockinView(APIView):
    ...
    def post(self, request, reservation_id=None):
        try:
            ...
            with transaction.atomic():
                data = request.data
                reservation_items = data.get("reservation_items", [])
                ...
                for item in reservation_items:
                    if not item.get("reservation_item_id"):
                    ...
                    for i in stock_list:
                        ...
                        if product.perishable_product:
                            expired_date = i.get("expired_date", None)
                            if not expired_date:
                                raise ValueError(
                                    "Perishable material can't be added directly without expired date."
                                )
...
class BarcodeView(APIView, CustomPagination):
    ...
    def validate_return_stock_barcode(self, barcode_no):
        ...
        if barcode.status != "Used":
            response = {
                "success": False,
                "message": "Barcode is not used yet",
            }
            return Response(response, status=400)
        ...
        if not histoty.exists():
            response = {
                "success": False,
                "message": "Invalid return stock barcode, no stock out history found for this material",
            }
            return Response(response, status=400)
...
class SearchRequisitionsView(APIView):
    serializer_class = SearchRequisitionsSerializer
    ...
    def get(self, request):
        try:
            ...
            response = {
                "success": True,
                "message": "Requisition List Get Succesfully",
                "data": data,
            }
            return Response(response, status=200)
        except Exception as e:
...
class SearchReservationsView(APIView):
    serializer_class = SearchReservationtionsSerializer
    ...
    def get(self, request):
        try:
            ...
            response = {
                "success": True,
                "message": "Reservation List Get Succesfully",
                "data": data,
            }
            return Response(response, status=200)
        except Exception as e:
