...
class RackStocksSerializer(serializers.ModelSerializer):
    def to_representation(self, obj):
        ...
        ret["expired_date"] = (
            obj.expired_date.strftime("%d %b %Y") if obj.expired_date else None
        )
        return ret
...
class RacksProductDetailsSerializer(serializers.ModelSerializer):
    def to_representation(self, obj):
        ...
        rack_stock = Stocks.objects.select_related("product", "rack").filter(
            product=obj, rack__id=rack_id, quantity__gt=0
        )
...
class ProductStocksSerializer(serializers.ModelSerializer):
    def to_representation(self, obj):
        ...
        ret["rack"] = {"rack_no": obj.rack.rack_no, "id": obj.rack.id}
        ret["barcode"] = obj.barcode.barcode_no
        ret["created_at"] = obj.created_at.strftime("%d %b %Y")
        ret["expired_date"] = (
            obj.expired_date.strftime("%d %b %Y") if obj.expired_date else None
...
class StocksHistorySerializer(serializers.ModelSerializer):
    def to_representation(self, obj):
        ...
        ret["user"] = (
            {"name": obj.user.name, "email": obj.user.email} if obj.user else None
        )
        ret["employee"] = (
            {
                "name": obj.employee.name,
                "personnel_number": obj.employee.personnel_number,
            }
            if obj.employee
            else None
        )
        ret["stock"] = {
            ...
            "rack": (
                {"id": obj.stock.rack.id, "rack_no": obj.stock.rack.rack_no}
                if obj.stock.rack
                else None
            ),
...
class BarcodeSerializer(serializers.ModelSerializer):
    def to_representation(self, obj):
        ...
        ret["product"] = {"id": product.id, "name": product.name} if product else None
        ret["rack"] = (
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "mobile_number",
            "personnel_number",
            "is_allow_email_daily_report",
            "is_allow_email_low_stock_alert",
            "user_permission",
        ]
        read_only_fields = fields
...
class ReservationItemStockinSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, obj):
        ...
        ret["source"] = (
            {"id": obj.source.id, "name": obj.source.name} if obj.source else None
        )
        return ret
...
class RequisitionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if self.context.get("include_items", False):
            ret["items"] = RequisitionItemSerializer(
...
class ReservationSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if self.context.get("include_items", False):
            ret["items"] = ReservationItemSerializer(
                instance.items.all(), many=True
            ).data
        return ret
...
class SearchMaterialSerializer(serializers.ModelSerializer):
    def to_representation(self, obj):
        ...
        if self.context.get("include_net_quantity", False):
            ret["label"] = (
                f"{obj.name} (NQ: {int(obj.net_quantity)})\nUCS-{obj.ucs_code}"
            )
        else:
            ret["label"] = obj.name
        ret["value"] = obj.id
...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Employees
        fields = ["id", "name", "personnel_number", "phone", "shoe_size"]
...
class ReportEmployeeSerializer(serializers.Serializer):
    ...
    employee_personnel_number = serializers.CharField(
        source="employee__personnel_number"
    )
    material_name = serializers.CharField(source="stock__product__name")
    ucs_code = serializers.CharField(source="stock__product__ucs_code")
    total_stock_out = serializers.IntegerField()
