...
def train_model_on_weekly_basis():
    ...
    for product_name, product_df in df.groupby('product_name'):
        if len(product_df) < 2:
            print(f"Skipping model training for '{product_name}': Only {len(product_df)} month(s) of history found (Prophet requires at least 2).")
            continue

        if product_df['y'].isnull().sum() >= len(product_df) - 1:
            print(f"Skipping model training for '{product_name}': Insufficient non-NaN data.")
            continue
