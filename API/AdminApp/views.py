...
class DashboardView(APIView):
    ...
    def get_perishable_stock(self):
        ...
        print("datewise_count", datewise_count)
    ...
    def get(self, request, id=None):
        try:
            ...
            if barcode := request.GET.get("barcode", None):
                if stock := Stocks.objects.filter(
...
class ProductsStockView(APIView, CustomPagination):
    ...
    def get(self, request, id=None):
        try:
            ...
            instance = (
                Stocks.objects.select_related("rack", "barcode")
                ...
                if product := Products.objects.filter(id=id).first():
                    ...
                    if query := request.GET.get("query"):
                        f &= Q(
                            ...
                            | Q(stock__source__name__icontains=query)
...
class BarcodeView(APIView, CustomPagination):
    ...
    def validate_return_stock_barcode(self, barcode_no):
        ...
        data = {
            ...
            "source": {"value": stock.source.id, "label": stock.source.name},
        }
...
class SearchMaterialView(APIView):
    ...
    def get(self, request):
        try:
            instance = Products.objects.all().only("name", "id").order_by("name")
            response = {
                "success": True,
...
class GenerateStockData(APIView):
    ...
    def post(self, request):
        try:
            ...
            from InventoryManagementSystems.AdminApp.scripts.material_rack import (
                stock_in_out,
            )
