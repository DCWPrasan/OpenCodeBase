    ...
    def get(self, request, id=None):
        try:
            if id:
                ...
                instance = SIR.objects.filter(filter_criteria).order_by("department__name").distinct()
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if instance.department != department:
                    log_details += f"Department : {instance.department.name if instance.department else None} ➡️ {department.name if department else None} |"
                    instance.department = department
                    
                if instance.unit != unit:
                    log_details += f"Unit : {instance.unit.name if instance.unit else None} ➡️ {unit.name if unit else None} |"
                    instance.unit = unit
    ...
    def get(self, request, id=None):
        try:
            if id:
                ...
                instance = StabilityCertification.objects.filter(filter_criteria).order_by("department__name").distinct()
    ...
    def get(self, request, id=None):
        try:
            if id:
                ...
                instance = Compliance.objects.filter(filter_criteria).order_by("department__name").distinct()
...
class DocumentApiView(APIView):
        ...
        try:
            if id:
                try:
                    ...
                    instance = Document.objects.get(filter_criteria)
                ...
                instance = Document.objects.filter(filter_criteria).order_by("-created_at").distinct()
    ...
    def post(self, request):
        try:
            data = request.data
            document_number = data.get("document_number", None) or None
            description = data.get("description", None) or None
            attachment = data.get("file", None) or None
 
            if not all([document_number, description, attachment]):
                response = {
                    "success": False,
                    "message": "All the mandatory fields are required."
                }
    ...
    def put(self, request):
        try:
            ...
            if Document.objects.filter(document_number=document_number).exclude(document_number=instance.document_number).exists():
                response = {
                    "success": False,
                    "message": "Document Number already exist."
                }
            ...
            log_details = ""
            with transaction.atomic():
                if instance.document_number != document_number:
                    log_details += f"Reference Number : {instance.document_number} ➡️ {document_number} |"
...
class ArchiveDocumentApiView(APIView):
        ...
        try:
            ...
            if is_archive:
                ...
                DocumentLog.objects.create(
                    ...
                    details = f"Archive reason : {document.archive_reason} ➡️ None."
                )
            document.is_archive = is_archive
            document.archive_reason = archive_reason
            document.save()
            response = {
                "success": True,
    ...
    def put(self, request):
        try:
            ...
            for document in Document.objects.filter(id__in=document_id_list, is_archive=True):
                DocumentLog.objects.create(
                    user = request.user,
                    document = document,
                    status = "Update Document",
...
class DocumentLogApiView(APIView):
    pagination_class = CustomPagination
    ...
    def get(self, request,doc_id=None):
        try:
            ...
            status = request.GET.get("status", None)
            user_id = request.GET.get("user", None)
            document_list = request.GET.get("document", None)
            if doc_id:
                try:
                    if request.user.role == "User":
                        document = Document.objects.get(id=doc_id, is_archive=False)
                    else:
                        document = Document.objects.get(id=doc_id)
                    filter_criteria &= Q(document=document)
                    if query:
                        filter_criteria &= Q(
                            Q(user__full_name__icontains=query)
                            | Q(user__personnel_number__icontains=query)
                ...
                if query:
                    filter_criteria &= Q(
                        Q(document_document_number__icontains=query)
                        | Q(user__full_name__icontains=query)
                        | Q(user__personnel_number__icontains=query)
                        | Q(message__icontains=query)
                    )
            ...
            if document_list:
                filter_criteria &= Q(document__id__in=document_list.split(','))
            ...
            if user_id :
                filter_criteria &= Q(user__id__in=user_id.split(','))
            instance = (
                DocumentLog.objects.select_related("user").filter(filter_criteria).order_by("-action_time")
            )
...
class DownloadDocumentFileApiView(APIView):
    def get(self, request, id):
        try:
            try:
...
class DownloadDocumentLogExcelApiView(APIView):
    serializer_class = DocumentLogExcelSerializer
    ...
    def get(self, request):
        try:
            ...
            if user_id :
                filter_criteria &= Q(user__id__in=user_id.split(','))
            instance = (
                DocumentLog.objects.select_related().filter(filter_criteria).order_by("-action_time")
            )
...
class BulkArchiveDocumentApiView(APIView):
    serializer_class = DocumentSerializer
    ...
    def get(self, request):
        try:
            ...
            instance = Document.objects.filter(filter_criteria).order_by("-created_at")
    ...
    def put(self, request):
        try:
            ...
            for document in Document.objects.filter(id__in=document_id_list, is_archive=False):
...
class DocumentPermanentDeleteApiView(APIView):
    @allowed_superadmin
    def post(self, request):
        try:
            data = request.data
            ...
            document = Document.objects.filter(id__in=document_id_list, is_archive=True)
            if not document:
                response = {
                    "success": False,
                    "message": "No Compliance found."
                ...
                return Response(response, status=400)
            for i in document:
                i.documentlog_set.annotate(
                        new_message=Concat(F("message"),Value(f" ({i.document_number} DELETED)"),)
                    ).update(message=F("new_message"))
                DocumentLog.objects.create(
                    user = request.user,
                    document = None,
                    status = "Delete Document",
                    message = f"Document {i.document_number} deleted.",
                    details = f"Document {i.document_number} DELETED."
                )
                # attachment deletion will handle by model signal
                i.delete()
            response = {
