...
class DashboardDrawingUploadGraphView(APIView):
    def get(self, request):
        try:
            ...
            filter_criteria = Q(created_at__date__in=date_list)
            upload_items = Drawing.objects.filter(filter_criteria).values("created_at__date").annotate(view=Coalesce(Count("id"), 0))

            log_dict = {item["created_at__date"]: item for item in upload_items}
